...
const lumirLinuxSnapLayer: Project = {
  ...
  layerLabel: { ko: "분석 플랫폼", en: "Analysis Platform" },
  layerIcon: "⚙",
  title: {
  ...
  impact: {
    ko: "ISCE2 도입으로 처리 속도를 확보해 3 레이어 통합 서비스의 분석 플랫폼이 가능해졌습니다. 광교산 시루봉 -17.30 mm/yr (GNSS 검증) + PSI 5,143,119 PS + 5m DEM TC + 사업 보고서 v1~v4를 AI native로 작성 시간이 0에 가까웠습니다. 또한 외부 플랫폼이 요구한 'XYZ 좌표 시계열'을 전달하면서, 풀 시계열 일괄 응답(81MB·13s)을 시점 스냅샷 바이너리 + 점클릭 분리로 재설계해 1.1MB·0.56s로 줄였습니다 — DB 쿼리는 73ms라 병목이 직렬화·전송임을 측정으로 진단한 뒤 페이로드 구조를 바꾼 결과입니다.",
    en: "Introducing ISCE2 unlocked the processing speed needed to make the analysis platform of the 3-layer service viable. Outputs: Mt. Sirubong −17.30 mm/yr subsidence (GNSS-verified), 5,143,119 PSI persistent scatterers, 5 m DEM TC, and program reports v1–v4 — all AI-native, with near-zero writing time. I also delivered the external platform's required 'XYZ coordinate time series': re-architecting the full-series bulk response (81 MB · 13 s) into a per-epoch snapshot binary + point-click split brought it to 1.1 MB · 0.56 s — done by first measuring that the DB query was only 73 ms, so the bottleneck was serialization and transfer, then changing the payload structure.",
  },
  keywords: [
...
const sarSearchAndAnalyzerLayer: Project = {
  ...
  diagram: {
    caption: {
      ko: "사용자가 지도에서 지역·AOI를 골라 SAR 데이터를 검색하고 InSAR 분석을 요청하는 프론트엔드. Next.js Route Handler BFF가 Plan(mock)/Current(실제 API) 환경 분리로 데이터·운영 플랫폼과 분석 플랫폼(InSAR)을 호출하고, 결과를 사용자가 다운로드합니다. 관리자는 별도 콘솔로 운영 — 3 레이어 통합의 사용자 진입점입니다.",
      en: "The user-facing frontend where a user picks a region/AOI on the map to search SAR data and request InSAR analysis. A Next.js Route-Handler BFF (Plan-mock / Current-real env split) calls the data & ops platform and the analysis platform (InSAR); the user then downloads results. Admins run it from a separate console — this is the user entry point of the 3-layer service.",
    },
    nodes: [
      {
        id: "user",
      ...
      },
      {
        ...
        kind: "external",
        icon: "⚙",
        label: { ko: "분석 플랫폼", en: "Analysis Platform" },
  ...
  problem: {
    ko: "사내에서 위성 데이터를 지도에서 검색·요청하고 InSAR 분석까지 요청하는 통합 프론트 서버가 필요했습니다. 데이터·운영 플랫폼과 분석 플랫폼을 묶어주는 사용자 진입점입니다.",
    en: "We needed an internal frontend that lets users search satellite data on a map and request InSAR analysis in one place — the entry point that ties the data & ops platform and the analysis platform together.",
  },
  system: {
...
const lumirSarPlatform: Project = {
  ...
  system: {
    ko: "3 레이어 통합 서비스를 본인이 단독으로 설계·구축하고 있습니다. **데이터·운영 플랫폼**(sar-data-retrieval, NestJS 모노레포 + CDSE + NAS PoC + DDD 5-layer) + **분석 플랫폼**(lumir-linux-snap, 5종 도구 다중 스택 + 다중 agent 워크트리) + **프론트 레이어**(sar-search-and-analyzer, Next.js + 지도 + AOI + 분석 요청 UI). 모두 AI native 100%로 진행하며, [[집비치기-him]]에서 검증한 4-Layer + CQRS + 한글 메서드명 패턴을 회사 백엔드 설계에 역전이 중입니다.",
    en: "I'm solo-designing and building the 3-layer integrated service. **Data & Ops Platform** (sar-data-retrieval, NestJS monorepo + CDSE + NAS PoC + DDD 5-layer) + **Analysis Platform** (lumir-linux-snap, 5-tool stack + multi-agent worktrees) + **Frontend layer** (sar-search-and-analyzer, Next.js + map + AOI + analysis-request UI). All three are 100% AI-native, and I'm back-transferring the 4-layer + CQRS + Korean-method-name pattern validated in him into the company backend design.",
  },
  impact: {
  ...
  diagram: {
    caption: {
      ko: "사용자 요청이 프론트 → 데이터·운영 플랫폼(NestJS) → 분석 플랫폼(FastAPI)을 좌→우로 거치고, InSAR 결과가 데이터·운영 플랫폼으로 되돌아와 사용자에게 응답되는 흐름. 데이터·운영 플랫폼(NestJS)은 분석 플랫폼(FastAPI)에서 InSAR 데이터를 API로 받아와 저장·중계합니다. 실선(주황)=정방향 요청, 점선=응답·데이터 환류. 한 사람이 3 레이어를 풀스택으로 묶고 있습니다.",
      en: "A user request flows left→right through frontend → data & ops platform (NestJS) → analysis platform (FastAPI); the InSAR results return to the data & ops platform to answer the user. The data & ops platform (NestJS) fetches InSAR data from the analysis platform (FastAPI) over an API, then stores and relays it. Solid (amber) = forward request, dashed = response / data feedback. One person owns all three layers full-stack.",
    },
    nodes: [
      {
        id: "user",
  ...
  images: [
    ...
    },
    {
      ...
      alt: {
        ko: "InSAR 시계열 (분석 플랫폼)",
        en: "InSAR time series (analysis platform)",
      },
      caption: {
        ko: "분석 플랫폼 — Control points 시계열 (65 epoch / 2.30 yr stack)",
