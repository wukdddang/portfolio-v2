 ...
 *
 * 인터랙션:
 *  - 클릭 포커스 트레이싱: 카드를 클릭하면 연결된 카드·엣지만 강조, 나머지는 흐림(다시/빈 곳 클릭으로 해제).
 *    (포커스 시 흐려지는 카드/엣지만 새 객체로 만들어 리렌더 churn·깜빡임을 최소화)
...
const STR = {
  ...
  hint: {
    ko: "🔍 레이어 박스 클릭 → 줌인 · 카드 클릭 → 연결 강조",
    en: "🔍 Click a layer box to zoom in · click a card to trace links",
  },
...
const INTER_FLOW: {
  ...
  // 프론트 '분석 레이어' 노드 → 데이터·운영 플랫폼 *박스*(하위 다이어그램).
  // 프론트 관점의 정방향 흐름이라 primary(주황 실선) — 백엔드 박스간 환류(④)와는 레벨이 다름.
  { from: "sar-search-and-analyzer", fromNode: "analysis", to: "sar-data-retrieval", label: { ko: "InSAR 데이터", en: "InSAR data" }, kind: "primary", fromSide: "right", toSide: "left" },
...
export function PlatformDiagram({ project }: { project: Project }) {
  ...
  const connected = useMemo(() => {
    ...
    if (focused) {
      ...
      for (const e of base.edges) {
        if (e.source === focused) s.add(e.target);
        if (e.target === focused) s.add(e.source);
      }
    }
    return s;
  }, [focused, base.edges]);

  // 박스 active 표시 — 드릴인 시에만 변함 (클릭 포커스와 무관 → 포커스가 박스 객체를 재생성하지 않음)
  const styledNodes = useMemo(() => {
    return base.nodes.map((n) => {
      // 카드는 클릭 시 포커스 트레이싱 → 클릭 가능 커서. 인라인 style로 (drillable 카드와 동일 방식 →
      // xyflow .react-flow__node 커서 규칙을 확실히 이김. className 유틸은 동일 specificity·소스순서로 짐)
      if (n.type === "card")
        return { ...n, style: { ...n.style, cursor: "pointer" } };
  ...
  const enter = useCallback(
    (boxId: string) => {
      setActiveBox(boxId);
      setFocused(null); // 레이어 진입 시 카드 포커스 해제
  ...
  const reset = useCallback(() => {
    setActiveBox(null);
    setFocused(null); // 전체 보기로 돌아가면 카드 포커스 해제
          ...
          <ReactFlow
            ...
            onNodeClick={(_, node) => {
              if (node.type === "layerBox") enter(node.id);
                ...
                setFocused((prev) => (prev === node.id ? null : node.id));
            }}
